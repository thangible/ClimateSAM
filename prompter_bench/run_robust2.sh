...
for args in "robust_decoder_box_s0 --seed 0" "robust_decoder_box_s1 --seed 1"; do
  ...
  .venv/bin/python -u prompter_bench/train_robust_decoder.py --encoder $ENC --wandb --mode box "$@" > results/logs/robust_${name}.log 2>&1 || echo "FAILED $name"
...
if [ ! -f results/runs/$ENC/$name/summary.json ]; then
  ...
  echo "$(date +%H:%M) start $name"
  .venv/bin/python -u prompter_bench/train_robust_decoder.py --encoder $ENC --wandb --mode hybrid --seed 0 --no_real > results/logs/robust_${name}.log 2>&1 || echo "FAILED $name"
  echo "$(date +%H:%M) done $name"
fi
