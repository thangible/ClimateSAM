...
class _LoRA_qkv_dual(nn.Module):
    """Dual-task qkv adapter: separate low-rank adapters for TC and AR.

    task_mode controls which adapters are added at forward time:
      - 'tc'  : apply only TC adapters
      - 'ar'  : apply only AR adapters
      - 'both': apply both (sum)
    """

    def __init__(self,
                 qkv: nn.Linear,
                 a_q_tc: nn.Linear, b_q_tc: nn.Linear,
                 a_v_tc: nn.Linear, b_v_tc: nn.Linear,
                 a_q_ar: nn.Linear, b_q_ar: nn.Linear,
                 a_v_ar: nn.Linear, b_v_ar: nn.Linear):
        super().__init__()
        if not isinstance(qkv, nn.Linear):
            raise TypeError("qkv must be nn.Linear")
        self.qkv = qkv
        ...
        self.b_v_ar = b_v_ar
        self.dim = qkv.in_features
        # task mode: 'both'|'tc'|'ar'
        self.task_mode = 'both'

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        qkv = self.qkv(x)
        # apply TC adapters if requested
        if self.task_mode in ('tc', 'both'):
            new_q_tc = self.b_q_tc(self.a_q_tc(x))
            new_v_tc = self.b_v_tc(self.a_v_tc(x))
            qkv[..., : self.dim] = qkv[..., : self.dim] + new_q_tc
            qkv[..., -self.dim :] = qkv[..., -self.dim :] + new_v_tc
        # apply AR adapters if requested
        if self.task_mode in ('ar', 'both'):
            new_q_ar = self.b_q_ar(self.a_q_ar(x))
            new_v_ar = self.b_v_ar(self.a_v_ar(x))
            qkv[..., : self.dim] = qkv[..., : self.dim] + new_q_ar
            qkv[..., -self.dim :] = qkv[..., -self.dim :] + new_v_ar
        return qkv
...
class LoRA_Sam(nn.Module):
    ...
    Always creates dual-task adapters (TC and AR) so encoder can produce
    task-specialised embeddings without runtime flags.
    """

    def __init__(self, sam_model: nn.Module, r: int = 4, lora_layers: Optional[List[int]] = None, dual: bool = True):
        super().__init__()
        assert r > 0
        self.sam = sam_model
        # Force dual adapters unconditionally
        self.dual = True
        ...
        # inject dual adapters for every selected block
        for i, blk in enumerate(blocks):
            ...
            # TC adapters
            a_q_tc = nn.Linear(dim, r, bias=False)
            b_q_tc = nn.Linear(r, dim, bias=False)
            a_v_tc = nn.Linear(dim, r, bias=False)
            b_v_tc = nn.Linear(r, dim, bias=False)
            # AR adapters
            a_q_ar = nn.Linear(dim, r, bias=False)
            b_q_ar = nn.Linear(r, dim, bias=False)
            a_v_ar = nn.Linear(dim, r, bias=False)
            b_v_ar = nn.Linear(r, dim, bias=False)
            # init
            for m in (a_q_tc, a_v_tc, a_q_ar, a_v_ar):
                nn.init.kaiming_uniform_(m.weight, a=math.sqrt(5))
            for m in (b_q_tc, b_v_tc, b_q_ar, b_v_ar):
                nn.init.zeros_(m.weight)
            # mark parameter tensors so helper can find them
            for m in (a_q_tc, b_q_tc, a_v_tc, b_v_tc, a_q_ar, b_q_ar, a_v_ar, b_v_ar):
                setattr(m.weight, 'is_lora', True)
            blk.attn.qkv = _LoRA_qkv_dual(w_qkv, a_q_tc, b_q_tc, a_v_tc, b_v_tc, a_q_ar, b_q_ar, a_v_ar, b_v_ar)
            self.adapters.extend([a_q_tc, b_q_tc, a_v_tc, b_v_tc, a_q_ar, b_q_ar, a_v_ar, b_v_ar])

    def num_adapters(self) -> int:
        return len(self.adapters)

    def set_task(self, task: str):
        """Set active task for dual adapters. task in {'tc','ar','both'}."""
        if task not in ('tc', 'ar', 'both'):
            raise ValueError("task must be 'tc','ar' or 'both'")
        # iterate blocks and set task_mode on dual qkv modules
        for blk in self.sam.image_encoder.blocks:
            if hasattr(blk.attn, 'qkv'):
                qkv = blk.attn.qkv
                if isinstance(qkv, _LoRA_qkv_dual):
                    qkv.task_mode = task
...
class LoRAClimateSAMVanilla(nn.Module):
    ...
    This variant always creates task-specialised LoRA adapters (TC vs AR) and
    exposes encoder outputs for both tasks: encoder_image_tc, encoder_image_ar,
    encoder_interm_tc, encoder_interm_ar.
    """
    ...
    def __init__(
        ...
        lora_task_specific: bool = True,  # kept for compatibility but ignored
    ):
        super().__init__()
        ...
        # always apply dual LoRA adapters to image encoder
        self.lora = LoRA_Sam(self.ori_sam, r=r, lora_layers=lora_layers, dual=True)
        if freeze_base:
        ...
        # placeholders for last encoded features (task-specific)
        self.encoder_image_tc = None
        self.encoder_image_ar = None
        self.encoder_interm_tc = None
        self.encoder_interm_ar = None

    def set_lora_task(self, task: str):
        """Helper to set active LoRA adapters across the encoder."""
        if hasattr(self, 'lora') and isinstance(self.lora, LoRA_Sam):
            self.lora.set_task(task)

    def encode_images(self, input: torch.Tensor):
        """Always compute task-specific embeddings for TC and AR and expose them."""
        ...
        # TC
        self.set_lora_task('tc')
        image_embeddings_tc, interm_embeddings_tc = self.image_encoder(imgs)
        # AR
        self.set_lora_task('ar')
        image_embeddings_ar, interm_embeddings_ar = self.image_encoder(imgs)
        # restore both
        self.set_lora_task('both')

        # store explicitly for convenience
        self.encoder_image_tc = image_embeddings_tc
        self.encoder_image_ar = image_embeddings_ar
        self.encoder_interm_tc = interm_embeddings_tc
        self.encoder_interm_ar = interm_embeddings_ar

        # return both sets (tc, ar, intermediate tc, intermediate ar, image_input, original sizes)
        return image_embeddings_tc, image_embeddings_ar, interm_embeddings_tc, interm_embeddings_ar, image_input, ori_img_size

    def forward(self,
                image_input,
                image_embeddings: Union[torch.Tensor, dict],
                interm_embeddings: Union[List[torch.Tensor], dict],
        ...
        # If caller provided both task embeddings as returned by encode_images, unpack them.
        if isinstance(image_embeddings, tuple) and len(image_embeddings) >= 2:
            image_embeddings_tc = image_embeddings[0]
            image_embeddings_ar = image_embeddings[1]
        elif isinstance(image_embeddings, dict):
            image_embeddings_tc = image_embeddings.get('tc')
            image_embeddings_ar = image_embeddings.get('ar')
        else:
            # fallback to stored attributes
            image_embeddings_tc = getattr(self, 'encoder_image_tc', None)
            image_embeddings_ar = getattr(self, 'encoder_image_ar', None)
        ...
        # ensure returned masks are squeezed to (H,W) per item to avoid extra dims
