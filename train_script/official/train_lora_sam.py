...
def main_worker(worker_id, worker_args):
    ...
    if worker_args.load_pretrained:
        image_encoder_path = os.path.join(worker_args.exp_dir, f"lora_single_{worker_args.sam_type}_{worker_args.lora_r}_{worker_args.run_name}.pth")
            ...
            if worker_args.load_pretrained or epoch > 1: 
                ...
                if (miou_tc + miou_ar) / 2 > best_miou_total:
                    ...
                    if getattr(worker_args, 'save_model', False) and epoch > 4:
                        ...
                        save_path = os.path.join(worker_args.exp_dir, f"lora_single_{worker_args.sam_type}_{worker_args.lora_r}_{worker_args.run_name}.pth")
                        phase_1_weights = {
