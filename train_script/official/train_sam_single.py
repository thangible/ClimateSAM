...
from model.climatesam_single import ClimateSAM
...
def main_worker(worker_id, worker_args):
    ...
    #         base_model.image_encoder.load_state_dict(phase_1_checkpoint['image_encoder'])
    #         print(f"Image encoder weights loaded from {image_encoder_path}")
    #         base_model.mask_decoder.load_state_dict(phase_1_checkpoint['mask_decoder'])
            ...
            if worker_args.load_pretrained or epoch > 1: 
                ...
                if (miou_tc + miou_ar) / 2 > best_miou_total:
                    ...
                    if worker_args.save_model and epoch > 4:
                        ...
                        base_model = model.module if hasattr(model, 'module') else model
                        phase_1_weights = {
                            'image_encoder': base_model.image_encoder.state_dict(),
                            'mask_decoder': base_model.mask_decoder.state_dict(),
