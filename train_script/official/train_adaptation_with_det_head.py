...
# python train_script/official/train_det_head.py --config input_config_official  --sam_type vit_b --image_encoder_mlp_ratio 1  --run_name det_head --encoder_weights_name best_weights/infused_token_vit_b_1.0_best_only_bbox --run_name DET_HEAD --load_pretrained --pretrained_name 
...
class TokenGatedDetectionHead(nn.Module):
    def __init__(self, embedding_dim=256, token_dim=32):
        super().__init__()
...
def train_one_epoch(epoch, train_dataloader, climatesam, prompter, optimizer, scheduler, device, local_rank, worker_args, max_epoch_num, scaler, gradient_accumulation_steps):
    # Enable training on phase 2 modules (input adapter, image encoder, mask decoder)
    climatesam.train(mode=True, phase=getattr(worker_args, 'phase', 2))
    prompter.train()
    
    batch_pbar = tqdm(total=len(train_dataloader), desc=f'Epoch {epoch}/{max_epoch_num}', leave=True) if local_rank == 0 else None
    epoch_loss_dict = {"total": 0, "det_total": 0, "mask_total": 0, "ar_conf": 0, "ar_box": 0, "tc_conf": 0, "tc_box": 0}
    
    for train_step, batch in enumerate(train_dataloader):
        batch = batch_to_cuda(batch, device)

        with torch.amp.autocast('cuda'):
            # 1. Extract SAM Embeddings (Now computing gradients)
            ...
            det_loss = ar_loss + tc_loss
            
            # 5. Forward SAM Mask Decoder using ground truth bounding boxes for stable training
            tc_masks, ar_masks, _ = climatesam.forward(
            ...
            mask_loss_dict = compute_climate_loss(
                ar_masks=ar_masks,
                tc_masks=tc_masks,
                ar_masks_gt=batch['ar_object_masks'],
                tc_masks_gt=batch['tc_object_masks'],
                device=device,
                worker_args=worker_args
            )
            
            mask_loss = mask_loss_dict.pop('total_loss_for_backward')
            
            # 7. Aggregate Total Loss
        ...
        epoch_loss_dict["det_total"] += det_loss.item()
        epoch_loss_dict["mask_total"] += mask_loss.item()
    ...
    if local_rank == 0 and worker_args.wandb:
        ...
        wandb.log({
            ...
            "train/loss_det_total": epoch_loss_dict["det_total"] / steps,
            "train/loss_mask_total": epoch_loss_dict["mask_total"] / steps,
...
def set_up_model(worker_args, device):
    ...
    if os.path.exists(weights_path):
        ...
        print(f"Pretrained weights not found at {weights_path}. Check the path if you intended to load them.")
    
    # Explicitly unfreeze the requested components for phase 2 training
    climatesam.train(mode=True, phase=getattr(worker_args, 'phase', 2), verbose=True)
    ...
    if worker_args.load_pretrained:
        ...
        if not os.path.exists(generator_path):
            ...
            print(f"Pretrained weights for prompt generator loaded from {generator_path}")
...
def main_worker(worker_id, worker_args):
    ...
    # Optimizer now trains the prompter AND the unfrozen parts of the climatesam model
    trainable_params = list(prompter.parameters()) + [p for p in climatesam.parameters() if p.requires_grad]
    optimizer = torch.optim.AdamW(trainable_params, lr=getattr(worker_args, 'lr', 1e-4), weight_decay=1e-4)
    ...
    for epoch in range(1, worker_args.max_epoch_num + 1):
        ...
        if epoch % getattr(worker_args, 'valid_per_epochs', 1) == 0 or epoch == worker_args.max_epoch_num:
            ...
            if avg_miou > best_miou:
                best_miou = avg_miou
                save_path = os.path.join(worker_args.exp_dir, f"det_head_and_encoder_{worker_args.run_name}.pth")
                
                # Save both the detection head and the updated SAM components
                torch.save({
                    'det_head': prompter.state_dict(),
                    'image_encoder': climatesam.image_encoder.state_dict(),
                    'mask_decoder': climatesam.mask_decoder.state_dict(),
                    'input_adapter': climatesam.input_adapter.state_dict(),
                }, save_path)
