...
class ProposeAndScorePipeline:
    ...
    def _process_grid(self, points, cgnet_image, points_per_batch, prompt_type, conf_thresh, target_class_id, accumulators):
        ...
        # If no points were generated by the smart grid, skip
        if points is None or len(points) == 0:
            return all_masks, all_scores, all_classes, all_boxes, torch.zeros((cgnet_image.shape[1], cgnet_image.shape[2]), device=self.device)
...
def validate_propose_and_score(train_dataloader, val_dataloader, ar_metrics, tc_metrics, pipeline, device, worker_args, max_samples=None):
    ...
    # 1. Generate ONLY Positive Smart Grids (No BG points needed!)
    ...
    with torch.no_grad():
        for val_step, batch in enumerate(val_dataloader):
            ...
            for b in range(B):
                ...
                # Note: removed bg_points from generate_objects signature
