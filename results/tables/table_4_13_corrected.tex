...
\end{tabular}
\end{adjustbox}
\caption{SAM with CG-Net as prompter, re-run on the 61 test images (Infused Token MLP 1.0, Phase-1 checkpoint of Table 4.12). ``orig. script'': image passed through ClimateSAM.set\_infer\_img, which skips the learned input adapter (test\_prompt\_effect.py); other columns: input adapter applied as in training. Ordered by TC IoU.}
\label{tab:cgnet_prompter_corrected}
\end{table}
