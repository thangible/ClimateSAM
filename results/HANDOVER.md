...
### 3.5 Re-evaluation of the author's own Chapter-4 results (`results/08_thesis_recheck/`, `results/latex/recheck.tex`)

Done after the benchmark: the saved checkpoints and wandb logs behind Tables 4.1–4.13 were re-evaluated with the
thesis' own pipeline. **This affects Section 4.2, not only 4.3:**
- The Phase-1 validation (`train_adaptation.validate_one_epoch` → `ClimateSAM.set_infer_img`) **skips the learned input
  adapter**. Linear models: negligible (rows reproduce within ±0.015). Nonlinear models: +0.04 to +0.13 mean FG IoU when
  the adapter is applied. The linear adapter is only +0.005 to +0.042 better (thesis: +0.06 to +0.17), so the Table 4.6
  conclusion and its "3×3 convolution blurs the structures" explanation must be rewritten (a small AR advantage of the
  linear adapter remains). Other nonlinear rows (Concat, Single, LoRA) are likely affected but their checkpoints are
  missing.
- Every Phase-1 number was **selected on the test set** (best mean test IoU, every 5 epochs). Harmless for the token
  adapters (≤ 0.01), decisive for **Single LoRA, whose TC IoU collapses** during training (r32: 0.63 → 0.10; r64:
  0.59 → 0.12). Section 4.2.2's rank argument must be qualified.
- "Random" prompt rows are not reproducible (new random mixture per evaluation); Table 4.7 shows the NOSMOOTH run;
  the text around Tables 4.2–4.4 contradicts the tables; Table 4.11 parameter counts are exact.
- Phase-2 author checkpoints: `generator_128_vit_b_bbox` reproduces its log exactly; the others were trained on
  `best_weights/infused_token_vitb_mlp1_best.pth`, which was overwritten afterwards, so they cannot be verified.
...
| Re-evaluation of the Chapter-4 checkpoints (corrects Sections 4.2.1 / 4.2.2) | `results/latex/recheck.tex` | draft |
...
2. **Correct Section 4.2 with `results/latex/recheck.tex`** (nonlinear adapter results, test-set selection, LoRA TC
   collapse; see §3.5 above).
3. **Remove or correct everything elsewhere in the thesis that relies on the old Table 4.13** (abstract, introduction,
...
4. State anywhere relevant that model selection now uses a held-out validation split (the original scripts used the
   test set).
5. Keep the research-question structure (RQ1–RQ5) or fold it into the thesis' own framing; the conclusions of
   `REPORT.md` §5 are the agreed answers.
6. Adapt the tone and length to the rest of the thesis; the drafts are dense.
