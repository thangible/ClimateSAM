...
trained on realistic, out-of-fold prompts, or on deliberately corrupted prompts with empty targets for false boxes,
becomes more tolerant of box errors and rejects some false prompts, but it cannot add information that the prompt does
not contain. Prompter and decoder read the same frozen image embedding; whatever the decoder could infer
...
\begin{itemize}
  ...
  validation images of this protocol; this affects only experiments that use its outputs on training images. The CG-Net
  re-trained from the official ClimateNet weights inherits this for its checkpoint selection (not for its test numbers);
  the CG-Net trained from scratch is unaffected.
...
\begin{enumerate}
  \item \emph{Ensembles and fusion of prompter families.} The ensemble of MPG and CG-Net is the best automatic mask;
  a larger same-family ensemble is the missing control, and a fusion that keeps the diversity of the two input types
  (unlike MPG with raw fields) may keep the gain at lower cost.
  \item \emph{Realistic prompts already in Phase~1.} The prompt-robust decoder was trained only in Phase~2 with a frozen
  prompt encoder. Training Phase~1 with corrupted and prompter prompts is the stronger version; given
  Section~\ref{sec:decoder_adaptation_results} its benefit for automatic prompting is likely small, but it would give a
  more forgiving interactive tool.
